...
export async function fetchAspectCatalog(
  backendUrl: string,
): Promise<AspectCatalogEntry[]> {
  ...
  if (!response.ok) {
    throw new ReliquaryError(
      `catalog fetch refused (${response.status})`,
      response.status,
    );
  }
...
export async function postIngest(
  ...
  if (response.status === 422) {
    ...
    throw new ReliquaryError(
      "the gate rejected the payload — parser needs work",
      422,
      body,
    );
  }
 ...
 */
export function resolveAspectId(
