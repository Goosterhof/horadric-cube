...
<template>
    <div class="space-y-3">
        <div v-if="entries.length === 0" class="vault-card text-center">
            ...
            <p class="text-hd-bone/60 text-sm mt-2">Captures will appear here as you press the hotkey during play.</p>
        </div>

        <div v-for="entry in entries" :key="entry.id" class="vault-card hover:border-hd-rune/70 transition-colors">
            <div class="flex items-start justify-between gap-3">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        <span :class="['vault-label border px-2 py-0.5', badgeClass(entry.state)]">
                            {{ entry.state }}
                        </span>
                        <span class="vault-label text-hd-bone/40">
                            {{ relativeTime(entry.capturedAt) }}
                        </span>
                        <span v-if="entry.elapsedMs" class="text-hd-bone/40 text-xs"> {{ entry.elapsedMs }}ms </span>
                    </div>

                    <div v-if="entry.payload" class="mt-2 vault-rune text-xs">
                        <template v-if="entry.payload.type === 'item'">
                            {{ entry.payload.name ?? 'Item' }} · {{ entry.payload.slot ?? '—' }} ·
                    ...
                    <details v-if="entry.rawText" class="mt-2">
                        <summary class="vault-label cursor-pointer hover:text-hd-ember">raw OCR</summary>
                        <pre class="font-mono text-xs text-hd-bone/60 mt-2 whitespace-pre-wrap">{{
                            entry.rawText
                        }}</pre>
                    </details>
                </div>
